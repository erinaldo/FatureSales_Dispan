...
{
    public partial class FormDemonstracaoBrindeVisao : AppLib.Windows.FormVisao
    {
        private static FormDemonstracaoBrindeVisao _instance = null;
        DateTime data;
        int qtde = 0;
        public static FormDemonstracaoBrindeVisao GetInstance()
        {
            if (_instance == null)
                _instance = new FormDemonstracaoBrindeVisao();
            return _instance;
        }

        // private bool AnexoItemMovimento;

        public FormDemonstracaoBrindeVisao()
        {
            InitializeComponent();
        }
        ...
        //            for (int ii = 0; ii < dt.Rows.Count; ii++)
        //            {
        //                email.Assunto = "Alteração do carregamento de número " + _dr["IDPROGRAMACAOCARREGAMENTO"].ToString() + " do Pedido de número " + dr["NUMEROMOV"].ToString();
        //                email.Para = dt.Rows[ii]["EMAIL"].ToString();
        //                //Busca as informações da mensagem
        //                sql = @"SELECT 
        //ZPROGRAMACAOCARREGAMENTO.IDPROGRAMACAOCARREGAMENTO, 
...
WHERE LISTA.IDMOV = ? and LISTA.NSEQITMMOV = ?";
        ...
        private void FormDemonstracaoBrindeVisao_Load(object sender, EventArgs e)
        {
            ////AnexoItemMovimento = false;
...
WHERE TITMMOV.CODCOLIGADA = TPRD.CODCOLIGADA
  ...
  AND TITMMOV.IDMOV = ?";
        ...
        {
            if (new AppLib.Security.Access().Processo(grid1.Conexao, "APP8006", AppLib.Context.Perfil))
            {
        ...
        {
            if (new AppLib.Security.Access().Processo(grid1.Conexao, "APP8008", AppLib.Context.Perfil))
            {
                System.Data.DataRowCollection drc = grid1.GetDataRows();
        ...
        private void grid1_Excluir(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Movimentos != null)
                {
                    ...
                    {
                        //Verifica se existe item no carregamento.
                ...
                {
                    MessageBox.Show("Não foi possível excluir o item do Carregamento.\nFavor verificar.", "Informação do Sistema.", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void FormDemonstracaoBrindeVisao_FormClosed(object sender, FormClosedEventArgs e)
        {
            _instance = null;
        }
