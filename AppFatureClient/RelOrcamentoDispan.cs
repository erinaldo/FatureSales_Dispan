...
namespace AppFatureClient
{
	...
	CASE
		TMOV.FRETECIFOUFOB
	WHEN '1' THEN CASE WHEN TMOV.CODTRA IS NULL THEN 'CIF' ELSE (SELECT TTRA.NOME FROM TTRA WHERE TTRA.CODCOLIGADA = TMOV.CODCOLIGADA AND TTRA.CODTRA = TMOV.CODTRA) + ' - CIF' END
	WHEN '2' THEN CASE WHEN TMOV.CODTRA IS NULL THEN 'FOB' ELSE (SELECT TTRA.NOME FROM TTRA WHERE TTRA.CODCOLIGADA = TMOV.CODCOLIGADA AND TTRA.CODTRA = TMOV.CODTRA) + ' - FOB' END
