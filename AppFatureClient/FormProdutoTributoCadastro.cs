...
{
    public partial class FormProdutoTributoCadastro : DevExpress.XtraEditors.XtraForm
    {
        #region PROPRIEDADES
        ...
        string idprd = string.Empty;
        string codetd = string.Empty;
        Boolean editar = false;

        Comissao.ParametrosComissao pc = new Comissao.ParametrosComissao();

        public FormProdutoTributoCadastro(string _cod, string _codetd)
        {
            InitializeComponent();
            ...
            idprd = _cod;
            codetd = _codetd;
            editar = String.IsNullOrWhiteSpace(_codetd) ? false : true;
        }
        ...
        private void Atualizar()
        {
            try
            {
                string sql = String.Format(@"SELECT CODCOLIGADA,
                                                   ...
                                                   RECCREATEDON
                                            from TPRDCODFISCAL
                                            where IDPRD = {0} and CODETD = '{1}'", idprd, codetd);

                using (DataTable dt = MetodosSQL.GetDT(sql))
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        if (row["CODETD"] != DBNull.Value) { clEstado.Set((string)row["CODETD"]); }
                        if (row["CODFISCAL"] != DBNull.Value) { txtCodFiscal.Set((string)row["CODFISCAL"]); }
                        if (row["ALIQINTICMS"] != DBNull.Value) { decimalAliquotaInternaICMS.Set((decimal)row["ALIQINTICMS"]); }
                        if (row["FATORREDUCAOICMS"] != DBNull.Value) { decimalPorcentagemReducaoICMS.Set((decimal)row["FATORREDUCAOICMS"]); }
                        if (row["FATORSUBST"] != DBNull.Value) { decimalPorcentagemMargemST.Set((decimal)row["FATORSUBST"]); }
                        if (row["TRIBUTADOICMSST"] != DBNull.Value) { cbTributadoICMSST.Checked = row["TRIBUTADOICMSST"].ToString() == "1"; }
                        if (row["MODALIDADEBCICMS"] != DBNull.Value) { listaModalidadeICMS.Set(row["MODALIDADEBCICMS"].ToString()); }
                        if (row["MODALIDADEBCICMSST"] != DBNull.Value) { listaModalidadeICMSST.Set(row["MODALIDADEBCICMSST"].ToString()); }
                    }
                }
            }catch(Exception ex)
            {
        ...
        private void FormCadastro2_Load(object sender, EventArgs e)
        {
            ...
            {
                listaModalidadeICMS.Set(null);
                listaModalidadeICMSST.Set(null);
            }
        ...
        private void Salvar()
        {
            try
            {
                ...
                if (editar)
                {
                    sql = String.Format(@"update TPRDCODFISCAL 
                                               set CODFISCAL = '{2}', 
                                                   ALIQINTICMS = {3}, 
                                                   FATORREDUCAOICMS = {4}, 
                                                   TRIBUTADOICMSST = {5}, 
                                                   FATORSUBST = {6}, 
                                                   MODALIDADEBCICMS = {7}, 
                                                   MODALIDADEBCICMSST = {8}, 
                                                   RECMODIFIEDBY = '{9}', 
                                                   RECMODIFIEDON = getdate()
                                            where IDPRD = {0} and CODETD = '{1}'",
                                                    /*IDPRD*/ idprd,
                                                    /*CODETD*/ clEstado.Get(),
                                                    /*CODFISCAL*/ txtCodFiscal.Get() == null ? "" : txtCodFiscal.Get(),
                                                    /*ALIQINTICMS*/ decimalAliquotaInternaICMS.Get() == null ? "null" : decimalAliquotaInternaICMS.Get().ToString().Replace(",", "."),
                                                    /*FATORREDUCAOICMS*/ decimalPorcentagemReducaoICMS.Get() == null ? "null" : decimalPorcentagemReducaoICMS.Get().ToString().Replace(",", "."),
                                                    /*TRIBUTADOICMSST*/ cbTributadoICMSST.Checked ? 1 : 0,
                                                    /*FATORSUBST*/ decimalPorcentagemMargemST.Get() == null? "null" : decimalPorcentagemMargemST.Get().ToString().Replace(",", "."),
                                                    /*MODALIDADEBCICMS*/ listaModalidadeICMS.Get(),
                                                    /*MODALIDADEBCICMSST*/ listaModalidadeICMSST.Get(),
                                                    /*RECCREATEDBY*/ AppLib.Context.Usuario);
                }
                else
                {
                    sql = String.Format(@"insert into TPRDCODFISCAL (CODCOLIGADA, IDPRD, CODETD, CODFISCAL, ALIQINTICMS, FATORREDUCAOICMS, TRIBUTADOICMSST, FATORSUBST, MODALIDADEBCICMS, MODALIDADEBCICMSST, RECCREATEDBY, RECCREATEDON)
