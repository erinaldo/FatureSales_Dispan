...
{
    public partial class RelTabMaquina : DevExpress.XtraReports.UI.XtraReport
    {
        public string Conexao { get; set; }

        public RelTabMaquina()
        {
            InitializeComponent();
        }
...
TPRD.CODIGOPRD LIKE '01.001.%' AND
...
TPRD.CODCOLIGADA = ? ";
	...
	TPRD.CODIGOAUXILIAR, 
	TPRD.CODUNDVENDA, 
	RTRIM(CONVERT(CHAR,CONVERT(INT,ISNULL((SELECT ALIQUOTA FROM TTRBPRD WHERE CODCOLIGADA = TPRD.CODCOLIGADA AND IDPRD = TPRD.IDPRD AND CODTRB = 'IPI'),'0'))))+ '%' ALIQUOTA, 
	TPRD.CODIGOPRD, 
	TPRDCOMPOSTO.QUANTIDADE * ISNULL(TPRD.PRECO1,0) PRECO
FROM 
	TPRDCOMPOSTO
	INNER JOIN TPRD ON TPRDCOMPOSTO.CODCOLIGADA = TPRD.CODCOLIGADA AND TPRDCOMPOSTO.IDPRDCOMPONENTE = TPRD.IDPRD
	INNER JOIN TPRDCOMPL ON TPRDCOMPOSTO.CODCOLIGADA = TPRDCOMPL.CODCOLIGADA AND TPRDCOMPOSTO.IDPRDCOMPONENTE = TPRDCOMPL.IDPRD
WHERE 
	TPRDCOMPOSTO.IDPRD = ?  AND
	(TPRD.CODIGOAUXILIAR IS NULL OR TPRD.CODIGOAUXILIAR NOT IN (?)) AND
	TPRDCOMPL.TABELAPRECO = 'SIM' AND
	TPRD.ULTIMONIVEL = 1 AND
	TPRDCOMPOSTO.IDPRDCOMPONENTE <> ? AND
	TPRD.CODCOLIGADA = ?
ORDER BY TPRD.DESCRICAO
