...
{
    public partial class FormAlteraItemComposicao : DevExpress.XtraEditors.XtraForm
    {
        #region PROPRIEDADES
        ...
        public ItemComposicao RETORNO = new ItemComposicao();

        Comissao.ParametrosComissao pc = new Comissao.ParametrosComissao();

        public FormAlteraItemComposicao()
        {
            InitializeComponent();
        ...
        private void setConsulta()
        {
            clProduto.ColunaTabela = String.Format(@"(select IDPRD,
                                                        ...
                                                        from TPRODUTO 
                                                        where CODCOLPRD = 1 
                                                        AND SUBSTRING(CODIGOPRD, 1, 9) = SUBSTRING('{0}', 1, 9)) W", CODPRODUTO);
        }

        private void FormCadastro2_Load(object sender, EventArgs e)
        {
            setConsulta();
            clProduto.Set(CODPRODUTO);
            decimalQuantidade.Set(QUANTIDADE);
        ...
        private void simpleButtonOK_Click(object sender, EventArgs e)
        {
            try
            {
                string sql = String.Format(@"select IDPRD,
	                                                   ...
	                                                   CODIGOAUXILIAR,
	                                                   NOMEFANTASIA,
	                                                   (SELECT CODUNDVENDA FROM TPRODUTODEF WHERE CODCOLIGADA = TPRODUTO.CODCOLPRD AND IDPRD = TPRODUTO.IDPRD) UN
                                                from TPRODUTO 
                                                where CODCOLPRD = 1 
                ...
                {
                    RETORNO.CODCOLIGADA = 1;
                    RETORNO.CODFILIAL = 1;
                    RETORNO.IDPRD = (int)item["IDPRD"];
                    RETORNO.CODIGOPRD = (String)item["CODIGOPRD"];
                    RETORNO.CODIGOAUXILIAR = (String)item["CODIGOAUXILIAR"];
                    RETORNO.NOMEFANTASIA = (String)item["NOMEFANTASIA"];
                    RETORNO.QUANTIDADE = (decimal)decimalQuantidade.Get();
                    //RETORNO.UNIDADEMEDIDA = (String)item["UN"];
                    //RETORNO.PRECOUNITARIO = CalculoPreco.CacularPreco(RETORNO.IDPRD.ToString(), TIPOINOX, "", "").PRECO;
                }
