...
{
    public partial class FormRegraCFOPCadastro : DevExpress.XtraEditors.XtraForm
    {
        #region PROPRIEDADES
        ...
        public string cod = string.Empty;
        ...
        private void Carrega()
        {
            try
            {
                if(!String.IsNullOrWhiteSpace(cod))
                {
                    ...
                    sql = String.Format(@"select * from ZREGRACFOP where ID = {0}", cod);

                    DataTable dt = MetodosSQL.GetDT(sql);

                    foreach (DataRow row in dt.Rows)
                    {
                        clEstado.Set((String)row["CODETD"]);
                        clNumeroCCF.Set((String)row["NUMEROCCF"]);
                        clTipoProduto.Set((String)row["TIPOPRD"]);
                        clConsumo.Set((String)row["CODNATUSOCONSUMO"]);
                        clRevender.Set((String)row["CODNATREVENDER"]);
                        clIndustrializar.Set((String)row["CODNATINDUSTRIALIZAR"]);
                        clAtivo.Set((String)row["CODNATATIVOIMOB"]);

                        if (row["CODNATUSOCONSUMOSST"] != DBNull.Value)
                        {
                            clConsumoSemST.Set((String)row["CODNATUSOCONSUMOSST"]);
                        }

                        if (row["CODNATREVENDERSST"] != DBNull.Value)
                        {
                            clRevenderSemST.Set((String)row["CODNATREVENDERSST"]);
                        }            

                        cbNaoContrinuinte.Checked = row["NAOCONTRIBUINTE"].ToString() == "1";
                    }
                }
            }
            catch (Exception ex)
        ...
        private Boolean validar()
        {
            ...
            if (String.IsNullOrWhiteSpace(clEstado.textBoxDESCRICAO.Text) && clEstado.Enabled)
            {
                throw new Exception("Estado selecionado não é valido");
            }

            if (String.IsNullOrWhiteSpace(clTipoProduto.textBoxDESCRICAO.Text) && clTipoProduto.Enabled)
            {
                throw new Exception("Tipo de produto selecionado não é valido");
            }

            if (String.IsNullOrWhiteSpace(clConsumo.textBoxDESCRICAO.Text) && clConsumo.Enabled)
            {
                throw new Exception("CFOP do campo Consumo não é valida");
            }

            if (String.IsNullOrWhiteSpace(clRevender.textBoxDESCRICAO.Text) && clRevender.Enabled)
            {
                throw new Exception("CFOP do campo Revender não é valida");
            }

            if (String.IsNullOrWhiteSpace(clIndustrializar.textBoxDESCRICAO.Text) && clIndustrializar.Enabled)
            {
                throw new Exception("CFOP do campo Industrializar não é valida");
            }

            if (String.IsNullOrWhiteSpace(clAtivo.textBoxDESCRICAO.Text) && clAtivo.Enabled)
            {
                throw new Exception("CFOP do campo Ativo não é valida");
            }
        ...
        private Boolean Salvar()
        {
            try
            {
                ...
                if(editar)
                {
                    sql = String.Format(@"update ZREGRACFOP 
                                            set CODCOLIGADA = 1, 
	                                            CODFILIAL = 1, 
	                                            CODETD = '{0}', 
	                                            NUMEROCCF = '{1}', 
	                                            CODNATUSOCONSUMO = '{2}', 
	                                            CODNATREVENDER = '{3}', 
	                                            CODNATINDUSTRIALIZAR = '{4}', 
	                                            TIPOPRD = '{5}', 
	                                            CODNATATIVOIMOB = '{6}', 
	                                            NAOCONTRIBUINTE = {7},
                                                CODNATUSOCONSUMOSST = '{8}',
                                                CODNATREVENDERSST = '{9}'
                                            where ID = {10}",
                                    /*CODETD*/ clEstado.Get(),
                                    /*NUMEROCCF*/ clNumeroCCF.Get(),
                                    /*CODNATUSOCONSUMO*/ clConsumo.Get(),
                                    /*CODNATREVENDER*/ clRevender.Get(),
                                    /*CODNATINDUSTRIALIZAR*/ clIndustrializar.Get(),
                                    /*TIPOPRD*/ clTipoProduto.Get(),
                                    /*CODNATATIVOIMOB*/ clAtivo.Get(),
                                    /*NAOCONTRIBUINTE*/ cbNaoContrinuinte.Checked ? "1" : "0",
                                    /*CODNATUSOCONSUMOSST*/ clConsumoSemST.Get(),
                                    /*CODNATREVENDERSST*/ clRevenderSemST.Get(),
                                    /*ID*/ cod);
                    MetodosSQL.ExecQuery(sql);
                }
                else
                {
                    sql = String.Format(@"insert into ZREGRACFOP (CODCOLIGADA, CODFILIAL, CODETD, NUMEROCCF, CODNATUSOCONSUMO, CODNATREVENDER, CODNATINDUSTRIALIZAR, TIPOPRD, CODNATATIVOIMOB, NAOCONTRIBUINTE, CODNATUSOCONSUMOSST, CODNATREVENDERSST)
                                        values (1, 1, /*CODETD*/ '{0}', /*NUMEROCCF*/ '{1}', /*CODNATUSOCONSUMO*/ '{2}', /*CODNATREVENDER*/ '{3}',  
									 /*CODNATINDUSTRIALIZAR*/ '{4}', /*TIPOPRD*/ '{5}', /*CODNATATIVOIMOB*/ '{6}', /*NAOCONTRIBUINTE*/ {7}, /*CODNATUSOCONSUMOSST*/ '{8}', /*CODNATREVENDERSST*/ '{9}') select SCOPE_IDENTITY()",
                                     /*CODETD*/ clEstado.Get(),
                                     /*NUMEROCCF*/ clNumeroCCF.Get(),
                                     /*CODNATUSOCONSUMO*/ clConsumo.Get(),
                                     /*CODNATREVENDER*/ clRevender.Get(),
                    ...
                    cod = MetodosSQL.ExecScalar(sql).ToString();
                    editar = true;
                }
        ...
        private void cbNaoContrinuinte_CheckedChanged(object sender, EventArgs e)
        {
            clRevender.Enabled = !cbNaoContrinuinte.Checked;
            clRevenderSemST.Enabled = !cbNaoContrinuinte.Checked;
            clIndustrializar.Enabled = !cbNaoContrinuinte.Checked;
            clAtivo.Enabled = cbNaoContrinuinte.Checked;
        }

        #region Validação da saída dos componentes LookUp

        private void clEstado_Leave(object sender, EventArgs e)
        {
            if (clEstado.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clEstado))
                {
        ...
        private void clNumeroCCF_Leave(object sender, EventArgs e)
        {
            if (clNumeroCCF.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clNumeroCCF))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void clTipoProduto_Leave(object sender, EventArgs e)
        {
            if (clTipoProduto.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clTipoProduto))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void clConsumo_Leave(object sender, EventArgs e)
        {
            if (clConsumo.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clConsumo))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void clConsumoSemST_Leave(object sender, EventArgs e)
        {
            if (clConsumoSemST.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clConsumoSemST))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void clRevender_Leave(object sender, EventArgs e)
        {
            if (clRevender.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clRevender))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void clRevenderSemST_Leave(object sender, EventArgs e)
        {
            if (clRevenderSemST.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clRevenderSemST))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void clIndustrializar_Leave(object sender, EventArgs e)
        {
            if (clIndustrializar.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clIndustrializar))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void clAtivo_Leave(object sender, EventArgs e)
        {
            if (clAtivo.textBoxCODIGO.EditValue != null)
            {
                AppFatureClient.New.Class.Utilities util = new AppFatureClient.New.Class.Utilities();

                if (!util.ExisteCampo(clAtivo))
                {
                    XtraMessageBox.Show("Informe o valor correto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
