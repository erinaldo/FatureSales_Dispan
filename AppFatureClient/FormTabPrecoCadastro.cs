...
{
    public partial class FormTabPrecoCadastro : DevExpress.XtraEditors.XtraForm
    {
        #region PROPRIEDADES
        ...
        private void AtualizaGrid()
        {
            if (copiar)
            {
                string sql = String.Format(@"select * from ZTPRODUTOTABPRECOCOMPL where ID = '{0}'", codCopiar);
                gridData1.gridControl1.DataSource = MetodosSQL.GetDT(sql);
            }
            else
            {
                gridData1.Consulta = String.Format(@"select * from ZTPRODUTOTABPRECOCOMPL where ID = '{0}'", cod).Split(' ');
                gridData1.Atualizar();
            }
        ...
        private void CarregaDados()
        {
            try
            {
                DataTable dt = MetodosSQL.GetDT(String.Format(@"select * from ZTPRODUTOTABPRECO where ID = {0}", copiar ? codCopiar : cod));

                foreach (DataRow row in dt.Rows)
                {
                    txtProduto.Set((String)row["CODIGO"]);
                    listaAcabamento.Set((String)row["ACABAMENTO"]);
                    if (!copiar)
                    {
                        dtInicio.Set((DateTime)row["INICIOVIGENCIA"]);
                        dtFim.Set((DateTime)row["FIMVIGENCIA"]);
                    }
                    listaTipo.Set(row["TIPO"] != DBNull.Value ? (String)row["TIPO"] : null);
                }
            }
            catch (Exception ex)
            {
        ...
        private void FormCadastro2_Load(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrWhiteSpace(codCopiar))
            {
                copiar = true;
                gridData1.Enabled = true;
                CarregaDados();
                AtualizaGrid();
            }
            else if (String.IsNullOrWhiteSpace(cod))
            {
                editar = false;
            }
            else
            {
                ...
                gridData1.gridView1.OptionsBehavior.Editable = true;
                gridData1.gridView1.Columns["ID"].OptionsColumn.AllowEdit = false;
                gridData1.gridView1.Columns["CODCOLIGADA"].OptionsColumn.AllowEdit = false;
                gridData1.gridView1.Columns["CHAPA"].OptionsColumn.AllowEdit = false;
                gridData1.gridView1.Columns["DESCRICAOCHAPA"].OptionsColumn.AllowEdit = false;
                gridData1.gridView1.Columns["PESO"].OptionsColumn.AllowEdit = false;
            }
        ...
        private void Validar()
        {
            try
            {
                String sql = String.Format(@"select case when count(1) > 0 then 'S' else 'N' end as 'CONFLITO' from ZTPRODUTOTABPRECO
                                where CONVERT(date,'{0}', 103) BETWEEN INICIOVIGENCIA and FIMVIGENCIA
                                and CODIGO = '{1}'
                                and ACABAMENTO = '{2}'
                                and TIPO = '{3}'", "{0}", txtProduto.Get(), listaAcabamento.Get(), listaTipo.Get());

                if (MetodosSQL.GetField(String.Format(sql, dtInicio.Get()), "CONFLITO") == "S")
                {
                    throw new Exception("Data Inicio está em conflito com outra vigencia");
                }

                if (MetodosSQL.GetField(String.Format(sql, dtFim.Get()), "CONFLITO") == "S")
                {
                    throw new Exception("Data Fim está em conflito com outra vigencia");
                }
        ...
        private Boolean Salvar()
        {
            try
            {
                ...
                string insert = @"insert into ZTPRODUTOTABPRECO values (/*CODCOLIGADA*/ 1, 
											 /*CODIGO*/ '{0}', 
											 /*ACABAMENTO*/ '{1}', 
											 /*INICIOVIGENCIA*/ CONVERT(date,'{2}', 103), 
											 /*FIMVIGENCIA*/ CONVERT(date,'{3}', 103),
                                             /*TIPO*/ {4},
                                             /*INATIVO*/ 0) select SCOPE_IDENTITY()";
                ...
                if (copiar)
                {
                    ...
                    sql = String.Format(insert,
                                             /*CODIGO*/ txtProduto.Get(),
                                             /*ACABAMENTO*/ listaAcabamento.Get(),
                                             /*INICIOVIGENCIA*/ dtInicio.Get(),
                                             /*FIMVIGENCIA*/ dtFim.Get(),
                                             /*TIPO*/ listaTipo.Get() == null ? "''" : String.Format("'{0}'", listaTipo.Get()));

                    cod = MetodosSQL.ExecScalar(sql).ToString();

                    DataTable dt = (DataTable)gridData1.gridControl1.DataSource;

                    List<String> itens = new List<string>();

                    foreach (DataRow row in dt.Rows)
                    {
                        sql = String.Format(@"insert into ZTPRODUTOTABPRECOCOMPL values (/*ID*/ '{0}',
										                                               /*CODCOLIGADA*/ 1, 
										                                               /*CHAPA*/ '{1}', 
										                                               /*DESCRICAOCHAPA*/ '{2}', 
										                                               /*PESO*/ {3}, 
                                                                                       ...
                                                                                       /*DESCRICAOCHAPA*/ row["DESCRICAOCHAPA"].ToString(),
                                                                                       /*PESO*/ row["PESO"].ToString().Replace(",", "."),
                                                                                       /*PRECO*/ row["PRECOKG"].ToString().Replace(",", "."));

                        itens.Add(sql);
                    }

                    MetodosSQL.ExecMultiple(itens);
                ...
                {
                    sql = String.Format(@"update ZTPRODUTOTABPRECO 
	                                       set CODCOLIGADA = 1, 
		                                       CODIGO = '{0}', 
		                                       ACABAMENTO = '{1}', 
		                                       INICIOVIGENCIA = CONVERT(date,'{2}', 103), 
        ...
        {
            FormTabPrecoCadastroItem frm = new FormTabPrecoCadastroItem();
            frm.cod = cod;
            frm.ShowDialog();
            AtualizaGrid();
        }
        ...
        {
            FormTabPrecoCadastroItem frm = new FormTabPrecoCadastroItem();
            frm.cod = gridData1.GetDataRow()["ID"].ToString();
            frm.chapa = gridData1.GetDataRow()["CHAPA"].ToString();
            frm.ShowDialog();
            AtualizaGrid();
        }

        private void gridData1_Excluir_1(object sender, EventArgs e)
        {
            DialogResult r = MessageBox.Show("Deseja mesmo excluir o registro?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (r == DialogResult.Yes)
            {
                MetodosSQL.ExecQuery(String.Format(@"delete from ZTPRODUTOTABPRECOCOMPL where ID = {0} and CHAPA = '{1}'", gridData1.GetDataRow()["ID"].ToString(), gridData1.GetDataRow()["CHAPA"].ToString()));
                AtualizaGrid();
            }
        }
