...
{
    public partial class RelPedidoOff : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void Detalhe()
        {
            ...
            string sSql = @"SELECT 
FCFO.CODCFO, FCFO.NOME, FCFO.CONTATO, TMOV.DATAEMISSAO, TMOV.MSGFORNEC, TMOV.DATAENTREGA, FCFO.SUFRAMA,
CASE TMOVCOMPL.TIPOVENDA WHEN 'C' THEN 'VENDA DIRETA'
WHEN 'R' THEN 'REVENDA' ELSE NULL END TIPOVENDA,
ISNULL(TMOV.VALORFRETE,0) VALORFRETE, ISNULL(TMOV.VALORDESP,0) VALORDESP, ISNULL(TMOV.VALOREXTRA1,0) VALOREXTRA1, ISNULL(TMOV.VALORDESC,0) VALORDESC, TMOV.RECCREATEDBY USUARIOCRIACAO, ISNULL(PERCENTUALDESC,0) PERCENTUALDESC,
RUAPRINCIPAL.DESCRICAO DESTIPORUA, FCFO.RUA, FCFO.NUMERO, BAIRROPRINCIPAL.DESCRICAO DESTIPOBAIRRO, FCFO.BAIRRO, TMOV.IDMOV NUMEROMOV, TMOV.HISTORICOLONGO, TMOV.NORDEM,
...
FCFO.EMAILENTREGA, TMOV.TPCULTURA, FCFO.CIDENTIDADE, FCFO.INSCRESTADUAL, TMOV.CONDPGTO, TMOV.FINANCIADO, TTRA.NOME NOMETRA, TRPR.NOME NOMERPR, TMOV.CAMPOLIVRE1, TMOV.PRAZOENTREGA, FRETECIFOUFOB,

CASE WHEN TMOV.FINANCIADO = 'NÃO' THEN 'Descrição das Mercadorias Alienadas com Reserva de Domínio' ELSE '' END MSGFINANCIADO
...
((TMOV.CODCFO IS NULL AND FCFO.CODCFO = TMOV.IDOFF) OR
 (NOT(TMOV.CODCFO) IS NULL AND FCFO.CODCFO = TMOV.CODCFO))";
            ...
            foreach (System.Data.DataRow row in dt.Rows)
            {
                ...
                txtEMAILENTREGA.Text = row["EMAILENTREGA"].ToString();
        ...
        private void Detalhe1()
        {
            ...
            string sSql = @"SELECT
TITMMOV.PROJETO SEQUENCIAL, TITMMOV.QUANTIDADE, TITMMOV.CODUND, TITMMOV.IDPRDCOMPOSTO, TITMMOV.PRECOUNITARIO, TITMMOV.QUANTIDADE * TITMMOV.PRECOUNITARIO AS VALORTOTAL,
...
ISNULL(CAST(TITMMOV.HISTORICOLONGO AS VARCHAR (5000)),'') +' '+ (SELECT('CODIGO:  '+ISNULL(TPRD.CODIGOPRD,'') +'     NCM:  '+ ISNULL(TPRD.NUMEROCCF,'')) FROM ZTPRODUTO TPRD WHERE IDPRD = TITMMOV.IDPRD AND CODCOLPRD = TITMMOV.CODCOLIGADA) + ' - ' + (SELECT 
		CASE 
...
WHERE TPRD.CODCOLPRD = TPRDCOMPL.CODCOLIGADA 
...
TT.CODUND CODUNDVENDA, TT.QUANTIDADE, TT.PRECOUNITARIO, TT.NSEQITMMOV, TT.IDPRDCOMPOSTO, TT.QUANTIDADE * TT.PRECOUNITARIO AS VALORTOTAL,
...
ORDER BY TT.IDMOV,TT.IDPRDCOMPOSTO,TT.NSEQITMMOV";
        ...
        private void Detalhe3()
        {
            ...
            string sSql = @"SELECT TT.PROJETO SEQUENCIAL, TT.QUANTIDADE QUANTIDADETOTAL, TT.CODUND CODUNDVENDA,
...
TT.PRECOUNITARIO,
(TT.QUANTIDADE *  TT.PRECOUNITARIO) VALORTOTAL,
TM.IDMOV,
TT.NSEQITMMOV,
TT.NSEQITMMOV,
TT.ALIQUOTAIPI,
