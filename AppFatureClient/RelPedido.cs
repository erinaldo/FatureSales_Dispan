...
{
    public partial class RelPedido : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void Detalhe()
        {
            ...
            string IdMov = AppLib.Context.poolConnection.Get(this.Conexao).ExecGetField(null, "select IDMOV from TMOV where CODTMV = '2.1.10' and NUMEROMOV = ?", new Object[] { SelectRow["NUMEROMOV"].ToString() }).ToString();
...
ORDER BY TT.NUMEROSEQUENCIAL";
        ...
        private void DetailReport9_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            string numeromov = AppLib.Context.poolConnection.Get(this.Conexao).ExecGetField(null, "select NUMEROMOV from tmov where codcoligada = ? and idmov = ?", new Object[] { AppLib.Context.Empresa, SelectRow["IDMOV"].ToString() }).ToString();
            dtParcelas = AppLib.Context.poolConnection.Get().ExecQuery(@"SELECT NUMEROPARCELA, VENCIMENTO, VALOR, FTB2.DESCRICAO 
FROM ZPARCELAS 
LEFT OUTER JOIN FTB2 
ON ZPARCELAS.CODTB2FLX = FTB2.CODTB2FLX 
AND ZPARCELAS.CODCOLIGADA = FTB2.CODCOLIGADA 
inner join TMOV TM
on TM.IDMOV = ZPARCELAS.IDMOV
WHERE ZPARCELAS.CODCOLIGADA = ? 
AND ZPARCELAS.NUMEROMOV = ? and ZPARCELAS.CODTMV = '2.1.10'", new object[] { AppLib.Context.Empresa, numeromov });
