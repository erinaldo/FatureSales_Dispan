...
{
    public partial class FormRegraProdutoCadastro : DevExpress.XtraEditors.XtraForm
    {
        #region PROPRIEDADES
        ...
        public FormRegraProdutoCadastro()
        {
            InitializeComponent();
        ...
        private void Carrega()
        {
            try
            {
                sql = String.Format(@"select * from ZTPRODUTOREGRA where CODCOLIGADA = 1 and CODFILIAL = 1 and CODDP = '{0}'", cod);

                DataTable dt = MetodosSQL.GetDT(sql);

                foreach (DataRow row in dt.Rows)
                {
                    campoDP.Set(row["CODDP"].ToString());
                    largura.Checked = (int)row["FLAGLARGURA"] == 1 ? true : false;
                    altura.Checked = (int)row["FLAGALTURA"] == 1 ? true : false;
                    comprimento.Checked = (int)row["FLAGCOMPRIMENTO"] == 1 ? true : false;
                    distanciamento.Checked = (int)row["FLAGDISTANCIAMENTO"] == 1 ? true : false;
                    chapa.Checked = (int)row["FLAGCHAPA"] == 1 ? true : false;
                    acabamento.Checked = (int)row["FLAGACABAMENTO"] == 1 ? true : false;
                    virola.Checked = (int)row["FLAGVIROLA"] == 1 ? true : false;
                    raio.Checked = (int)row["FLAGRAIO"] == 1 ? true : false;
                    tipoRaio.Checked = (int)row["FLAGTIPORAIO"] == 1 ? true : false;
                    septo.Checked = (int)row["FLAGSEPTO"] == 1 ? true : false;
                    complemento.Checked = (int)row["FLAGCOMPLEMENTO"] == 1 ? true : false;
                    ...
                    if (row["DATAVALPRECOFIXO"].ToString() != "01/01/1900 00:00:00")
                    {
                        if (row["DATAVALPRECOFIXO"] != DBNull.Value)
                        {
        ...
        private Boolean Salvar()
        {
            ...
            try
            {
                sql = String.Format(@"delete from ZTPRODUTOREGRA where CODCOLIGADA = 1 and CODFILIAL = 1 and CODDP = '{0}'", campoDP.Get());
                ...
                sql = String.Format(@"insert into ZTPRODUTOREGRA values (1, 1, /*CODDP*/ '{0}', /*LARGURA*/ '{1}', /*ALTURA*/ '{2}', /*COMPRIMENTO*/ '{3}', /*DISTANCIAMENTO*/ '{4}',
                                         ...
                                         /*COMPRIMENTO*/ comprimento.Checked ? 1 : 0,
                                         /*DISTANCIAMENTO*/ distanciamento.Checked ? 1 : 0,
                                         /*CHAPA*/ chapa.Checked ? 1 : 0,
                                         /*ACABAMENTO*/ acabamento.Checked ? 1 : 0,
                                         /*VIROLA*/ virola.Checked ? 1 : 0,
                                         /*RAIO*/ raio.Checked ? 1 : 0,
                                         /*TIPORAIO*/ tipoRaio.Checked ? 1 : 0,
                                         /*SEPTO*/ septo.Checked ? 1 : 0,
                                         /*COMPLEMENTO*/ complemento.Checked ? 1 : 0,
                                         /*PESO*/ 0,
        ...
        {
            tipoRaio.Checked = raio.Checked;
        }

        private void tipoRaio_CheckedChanged(object sender, EventArgs e)
        {
            raio.Checked = tipoRaio.Checked;
        }

        private void chkUsaPrecoFixo_CheckedChanged(object sender, EventArgs e)
        {
