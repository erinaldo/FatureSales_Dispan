...
{
    public partial class RelOrcamentoDesconto : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void Detalhe()
        {
            ...
            string sSql = @"SELECT 
FCFO.CODCFO, FCFO.NOME, FCFO.CONTATO, TMOV.DATAEMISSAO, TMOVCOMPL.MSGFORNEC, FCFO.SUFRAMA,
...
AND TMOV.IDMOV = TMOVHISTORICO.IDMOV";
        ...
        private void Detalhe1()
        {
            ...
            //            string sSql = @"SELECT
            //TITMMOVCOMPL.SEQUENCIAL, TITMMOV.QUANTIDADETOTAL, TITMMOV.CODUND, TITMMOV.IDPRDCOMPOSTO, 
            //(TITMMOV.PRECOUNITARIO - (TITMMOV.PRECOUNITARIO * TMOV.PERCENTUALDESC)/100) PRECOUNITARIO, 
            //(TITMMOV.QUANTIDADETOTAL * TITMMOV.PRECOUNITARIO) - ((((TITMMOV.QUANTIDADETOTAL * TITMMOV.PRECOUNITARIO) * TMOV.PERCENTUALDESC))/100) AS VALORTOTAL,
...
ROUND((ROUND(TITMMOV.QUANTIDADETOTAL,2) * ROUND(ROUND(TITMMOV.PRECOUNITARIO,4) - (ROUND(TITMMOV.PRECOUNITARIO,4) * ROUND(TMOV.PERCENTUALDESC,4)/100),4)),2) AS VALORTOTAL,
...
TT.CODUND CODUNDVENDA, TT.QUANTIDADETOTAL, TT.NSEQITMMOV, TT.IDPRDCOMPOSTO, 
...
WHERE    TT.CODCOLIGADA = TTRBMOV.CODCOLIGADA
     ...
     AND TT.NSEQITMMOV  = TTRBMOV.NSEQITMMOV
     AND TTRBMOV.CODTRB      = 'IPI'
),0))/100) * QUANTIDADETOTAL AS VALORIPI
...
ORDER BY TT.NUMEROSEQUENCIAL";
        ...
        private void DetailReport5_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            //Pega valor do banco e seta no campo. esse não está somando o valor dos itens no relatorio.
            txtTOTALGERAL.Text = string.Format("{0:n}", Convert.ToDecimal(AppLib.Context.poolConnection.Get().ExecGetField(string.Empty, "select ROUND(ISNULL((TM.VALORBRUTOORIG-TM.VALORDESC),0),2) as 'Valor' from TMOV TM where TM.CODCOLIGADA = ? and TM.IDMOV = ?", new object[] { AppLib.Context.Empresa, IdMov }))); 
            //txtSUBTOTAL.Text; 
            //string.Format("{0:n}", ((SubTotal + TotalIPI + Convert.ToDecimal(txtVALORDESP.Text) + Convert.ToDecimal(txtVALORFRETE.Text)) /*- Convert.ToDecimal(txtDESCONTOESPECIAL.Text)*/ - Convert.ToDecimal(txtDEVOLUCAO.Text)));
        }
