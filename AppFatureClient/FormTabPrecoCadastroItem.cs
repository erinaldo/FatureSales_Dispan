...
{
    public partial class FormTabPrecoCadastroItem : DevExpress.XtraEditors.XtraForm
    {
        #region PROPRIEDADES
        ...
        private void CarregaDados()
        {
            try
            {
                DataTable dt = MetodosSQL.GetDT(String.Format(@"select * from ZTPRODUTOTABPRECOCOMPL where ID = {0} and CHAPA = '{1}'", cod, chapa));

                foreach (DataRow row in dt.Rows)
                {
                    listaChapa.Set((String)row["CHAPA"]);
                    decimalPeso.Set((Decimal)row["PESO"]);
                    decimalPreco.Set((Decimal)row["PRECOKG"]);
                    txtDescricao.Set((String)row["DESCRICAOCHAPA"]);
                }
            }
            catch (Exception ex)
            {
        ...
        private void FormCadastro2_Load(object sender, EventArgs e)
        {
            ...
            if((String.IsNullOrWhiteSpace(cod)) || (String.IsNullOrWhiteSpace(chapa)))
            {
                editar = false;
            }
            else
            ...
            try
            {
                ...
                if (String.IsNullOrWhiteSpace(chapa))
                {
                    sql = String.Format(@"insert into ZTPRODUTOTABPRECOCOMPL values (/*ID*/ '{0}',
										                                               ...
										                                               /*PRECO*/ {4})",
                                                                                       /*ID*/ cod,
                                                                                       /*CHAPA*/ listaChapa.Get(),
                                                                                       /*DESCRICAOCHAPA*/ txtDescricao.Get(),
                                                                                       /*PESO*/ decimalPeso.Get().ToString().Replace(",", "."),
                                                                                       /*PRECO*/ decimalPreco.Get().ToString().Replace(",", "."));
                }
                else
                {
                    sql = String.Format(@"update ZTPRODUTOTABPRECOCOMPL 
                                            set CODCOLIGADA = 1, 
	                                            DESCRICAOCHAPA = '{2}', 
	                                            PESO = {3}, 
	                                            PRECOKG = {4}
        ...
        private void listaChapa_AposSelecao(object sender, EventArgs e)
        {
            try
            {
                txtDescricao.Set(listaChapa.comboBox1.Text);
                decimalPreco.Set(0);
                decimalPeso.Set(0);
            }
            catch (Exception ex)
